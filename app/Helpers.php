...
// for verification time 
 ...
 public function VerificationCommon($CheckExit,$saveArray){
              ...
              $data = [
                ...
                $GroupDetails = Group::where(['default_group' => $default_group])->get();
                   if(count($GroupDetails)>0){

                     foreach($GroupDetails as $v){

                            $get_group_users=explode(',',$v->user_id);
                        if(!(in_array($saveArray['user_id'],$get_group_users))){
                              if(count($get_group_users) > 0){
                              $Groups_ids=$v->user_id.','.$saveArray['user_id'];
                              }else{
                              $Groups_ids=$saveArray['user_id'];
                              }
                          }else{
                             $Groups_ids=$v->user_id;
                          }
                        $data = [
                        'user_id' => $Groups_ids,
                        ];
                        Group::where('id',$v->id)->update($data);
                        $Group_Members= GroupMember::where(['user_id' => $saveArray['user_id'],'group_id'=>$v->id])->first();
                        if(count($Group_Members)>0){
                          ...
                          // after this update the members count in groups table regarding this user
                            $total_user_count= $v->user_count +1;
                            $data = [
...
// send private message ===============
 ...
 public function  Save_DataPrivate($saveArray,$request){
           $BAC= [];

          $helper = new Helpers();

      $SaveArr = new MongoChat;
   if ($saveArray['type'] == "T" AND !empty($saveArray['message'])) 
          {
            $SaveArr['image'] = "";
            $SaveArr['lat'] ="";
          ...
          if ($saveArray['type'] == "I" AND ! empty($_FILES['image']['name'])) {
            ...
            $imagename =  $helper->upload($request->file('image'),'img/groupchatimg','none');
  ...
  public function  PrivateChat_Notification($saveArray,$currentPErsonDetails,$groupname,$date,$uploadedimage,$Reciver){
                  $helper = new Helpers();
                  $Type = $currentPErsonDetails->user_type;
                 if($Type == "G"){
                   $name = "Guest User";
                 ...
                 $sender_image=   $helper->get_USerImage($currentPErsonDetails);
                  $Reciver_image=   $helper->get_USerImage($Reciver);
               ...
               $message = array(
                    'message' => "You have a new Private Message from " . $name . " (" . $groupname->name . ")",
                    'sender_id' => $currentPErsonDetails->id,
                    'noti_for' => 'private',
                    'date' => $date,
                    'sender_name' => $currentPErsonDetails->name,
                    'sender_image' => $sender_image,
                    'friend_name' => $Reciver->name,
                    'name' => $currentPErsonDetails->name,
                    'friend_id' => $Reciver->id,
                    'friend_type' => $Reciver->user_type;
                    'friend_image' => $Reciver_image,
                    'message_img' => $uploadedimage,
...
public function  get_USerImage($currentPErsonDetails){

     if(!empty($currentPErsonDetails->image)){
                   if($currentPErsonDetails->register_type=="F"){
                     $sender_image = url('/') . "/images/users/" .$currentPErsonDetails->image;
                   }
                   if($currentPErsonDetails->register_type=="N"){
                    $sender_image = url('/') . "/images/users/" .$currentPErsonDetails->image;
                   }
                 }
             if(empty($currentPErsonDetails->image)){
                  $image =  url('/').'/images/users/no_image.png';
                $sender_image = $defaulturl;
             }
            ...
            if(count($CheckUSer)>0){
              ...
              // if not then add that memeber otherwise  
               ...
               if(count($GroupMemberData)>0){
                ...
                $GroupInformtionDAta=  Groupinformation::where(['group_id'=>$CheckGroup->id,'user_id'=>$id])->first();
                $date_time = date('Y-m-d H:i: A');
                $dat_tne =explode(" ",$date_time);
                $date =$dat_tne[0];
                $check_date =$date;
                if(count($GroupInformtionDAta)>0){
                  // update
                  $datamember = [
                  'user_id'  => $id,
                  'group_id'=>$CheckGroup->id,
                 ...
                 Groupinformation::where('id',$GroupInformtionDAta->id)->update($datamember);
                }else{
                  // insert in db
                 $data = new Groupinformation;
                 $data->group_id = $CheckGroup->id;
                 $data->user_id = $id;
...
//    add the user in default group =============


// pass only user id then add automatically in default group 
 public function AddDefaultGroup($userid){
                $default_group="1"; // set value 1 for default groups 
                $GroupDetails = Group::where(['default_group' => $default_group])->get();
                   ...
                   if(count($GroupDetails)>0){
                     ...
                     foreach($GroupDetails as $v){
                            $get_group_users=explode(',',$v->user_id);
                        if(!(in_array($userid,$get_group_users))){
                              if(count($get_group_users) > 0){
                              $Groups_ids=$v->user_id.','.$userid;
                              }else{
                              $Groups_ids=$userid;
                              }
                          }else{
                        ...
                        Group::where('id',$v->id)->update($data);
                        $Group_Members= GroupMember::where(['user_id' => $userid,'group_id'=>$v->id])->first();
                        if(count($Group_Members)>0){
                            //update
                            $data = [
                             'user_id' => $userid,
                          ...
                          Group::where('id',$v->id)->update($data);// pass only user id then add automatically in default group 
                        }
...
//============================== GEt Support chat =============================
 public function Support_PRivate_On($saveArray) {
          $helper = new Helpers();
 ...
 public function Support_PrivateOfftime_On($saveArray,$privateofftime) {
          $helper = new Helpers();
          $offset = $saveArray['page_no']*10;
            $limit = 10;
            $Get_chat =  Supportchat::offset($offset)->where('submit_time','<=',$privateofftime)->
            where(['sender_id'=>$saveArray['sender_id'],'group_id'=>$saveArray['group_id']])
           ->orwhere(['receiver_id' =>$saveArray['sender_id'],'group_id'=>$saveArray['group_id']])
           ->limit($limit)->with('SupportChatusers')->get();
           if(count($Get_chat)>0){
...
// Support get chat common function 
  public function SupportComman($Get_chat,$saveArray) {
                         $helper = new Helpers();
                         ...
                         $page_count = ceil($page_count);
                          foreach($Get_chat as $v){
                             ...
                             $data['senderid'] = $v->sender_id; 
                                 if(!empty($v->SupportChatusers->image)){
                                   if($v->SupportChatusers->register_type=="F"){
                                     $data['senderimage'] = $v->SupportChatusers->image;
                                   }
                                   if($v->SupportChatusers->register_type=="N"){
                                    $data['senderimage'] = url('/') . "/img/profile_images/" .$v->SupportChatusers->image;
                                   }
                                 }
                            if(empty($v->SupportChatusers->image)){
                           ...
                           $data['sendername'] = $v->SupportChatusers->name;
                             $data['msg'] = $v->message;
                            $data['type'] = $v->type;
                            if($v->type=="I"){
