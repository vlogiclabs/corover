...
#_____________________________________________________________________________#
 ...
 function update_deviceid()
 {
      ...
      $result=array('status'=>'1','message'=>'Success','User Id'=> $userExist['User']['id'],'activate_status'=>$userExist['User']['activate_status'],'services'=> $record1);
...
$result = array('status' => '1', 'message' => 'Successfully Sent', 'groupmsg_img' => $groupmsg_img);
   ...
   function save_group_badge($sender_id,$receiver_id,$group_id)
	...
	function report_abuse()
	{
		...
		if(!empty($saveArray['user_id'] AND $saveArray['friend_id']))
		{
			$condition_1 = "ReportAbuse.user_id='".$saveArray['user_id']."' AND ReportAbuse.friend_id='".$saveArray['friend_id']."'";
			$user_record_1 = $this->ReportAbuse->find('all',array('conditions'=> $condition_1));
			//print_r($condition_1);
			if(count($user_record_1) == '0')
			{
				
				$condition = "ReportAbuse.user_id='".$saveArray['user_id']."' AND ReportAbuse.friend_id='".$saveArray['friend_id']."'";
				$user_record = $this->ReportAbuse->find('all',array('conditions'=> $condition));
			
				if(count($user_record) < '2')
				{
					$save_data = ['user_id'=>$saveArray['user_id'] , 'friend_id'=>$saveArray['friend_id']];	
					$this->ReportAbuse->saveAll($save_data,array('validate'=>false));
					$result = array('status'=>'1','message'=>'Request Submitted');
					
					$condition_2 = "ReportAbuse.friend_id = '".$saveArray['friend_id']."' ";
					$user_record_again=$this->ReportAbuse->find('all',array('conditions'=> $condition_2));
					if(count($user_record_again) >= '2')
					{
						$status = "Deactivate";
						$this->User->updateAll(array('User.activate_status' => "'".$status."'"),array('User.id'=>$saveArray['friend_id']));
						$result = array('status'=>'1','message'=>'successfully');	
						
						/* Send Email To De-Activated user*/
						$condition_3 = "User.id = '".$saveArray['friend_id']."'";
						$user_record_1 = $this->User->find('first',array('conditions'=> $condition_3));
						
						$send_to_email = $user_record_1['User']['email'];
						$send_to_name = $user_record_1['User']['name'];
						...
						$this->Email->sendAs   = 'html';
						$this->Email->template = 'emailtemplate'; // note no '.ctp'
						$mes  = "Hello '".$name."' Your Account Has Been Deactivated.";
						$this->set('vars', array('name' => $name,'username' => $username,'subtitle' => $mes,'group_name' => $groupname, 'comment' => $comment ,'group_logo'=>$group_logo,'rating' => $rating ,'base' => BASE_URL,'email' => $email , 'sender_email' => $static_email ));	
			...
			{
				$result=array('status'=>'0','message'=>'Already reported by you!!');		
			}
		}
		else
		{
	...
	public function get_app_apk()
	{
		$saveArray = $this->data;
		if(!empty($saveArray['auth_key']))
		{
			//$user_check = $this->User->find('first', array("conditions" => array('User.id' => $saveArray['user_id'])));
				$app_apk_link =  BASE_URL . "app_apk/CoRover_Connect.apk";
				$result = array('status' => '1','message' => 'Successfully','apk_link'=>$app_apk_link);
	...
	public function download_corover_trips()
	{
		$saveArray = $this->data;
		if(!empty($saveArray['auth_key']))
		{
			//$user_check = $this->User->find('first', array("conditions" => array('User.id' => $saveArray['user_id'])));
				$app_apk_link =  BASE_URL . "app_apk/CoRover_Trips.apk";
				$result = array('status' => '1','message' => 'Successfully','apk_link'=>$app_apk_link);
